...
test('add a tour event', async ({ page }) => {
  ...
  await page.goto('http://localhost:3000/dashboard');
  await page
    .getByRole('row', { name: 'Parisian Art & Culture Tour' })
    .getByRole('cell')
    .nth(1)
    .click();
  await page.getByRole('cell', { name: 'Parisian Art & Culture Tour' }).click();
  ...
  await page
    .locator('form div')
    .filter({ hasText: 'EventselectEventNew York' })
    .getByRole('combobox')
    .selectOption('91c940b1-24e8-463f-96ef-f54f7e4aaf1d');
  await page
    .locator('form div')
    .filter({ hasText: 'hotelLabelselectHotelHôtel' })
    .getByRole('combobox')
    .selectOption('47c8f2e7-3d6b-402c-93b7-8a92ef43e6ab');
