...
await page.goto('http://localhost:3000/dashboard');
await page.getByRole('tab', { name: 'Tours' }).click();
await page.getByRole('row', { name: 'Arabian Desert Safari' }).getByRole('button').nth(1).click();
await page.getByRole('button', { name: 'Confirm' }).click();
await page.getByRole('button', { name: 'Create' }).click();
await page.getByRole('dialog').locator('input[type="text"]').click();
await page.getByRole('dialog').locator('input[type="text"]').fill('Arabian Desert Safari');
await page.locator('textarea').click();
await page.locator('textarea').fill('Embark on a luxurious desert adventure with private 4x4 rides, camel treks, and an evening of stargazing in Bedouin-style camps.');
await page.getByRole('button', { name: 'Save' }).click();
